...
// Construct contexts
auto nam_set = {GateType::h,          GateType::x,  GateType::rz,
                GateType::add,        GateType::cx, GateType::input_qubit,
...
auto tcount = [](Graph* graph) {
  return (float)(graph->specific_gate_count(GateType::t) +
                 graph->specific_gate_count(GateType::tdg));
};
...
extern "C" long unsigned int load_eqset_(const char* eqset_fn_,
                                         unsigned char** store) {
...
filter_greedy(Context* ctxt, std::vector<std::vector<CircuitSeq*>>& eccs) {
...
extern "C" long unsigned int load_greedy_xfers_(const char* eqset_fn_,
                                                unsigned char** store) {
  // std::string eqset_fn(eqset_fn_);
...
extern "C" void load_xfers_(const char* eqset_fn_, unsigned char** gstore,
                            long unsigned int* glen, unsigned char** astore,
                            long unsigned int* alen) {
  // std::string eqset_fn(eqset_fn_);
  std::string eqset_fn(reinterpret_cast<const char*>(eqset_fn_));
  std::istringstream iss(eqset_fn);
...
extern "C" int opt_circuit_(const char* cqasm_, int timeout, char* buffer,
                            int buff_size, unsigned char* xfers_) {
  std::string cqasm(cqasm_);

  std::vector<GraphXfer*>* xfers_ptr =
      reinterpret_cast<std::vector<GraphXfer*>*>(xfers_);
  ...
  // auto graph_after_search = graph.greedy_optimize(ctxt, eqset_fn,
  // /*print_message=*/ false);
  ...
  if (timeout == 0) {
    ...
    graph_after_search =
        graph->optimize(xfers, 1.05 * gcost_function(graph.get()), "", "",
                        false, nullptr, timeout);
  }
